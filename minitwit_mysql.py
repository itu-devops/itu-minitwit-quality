...
def connect_db():
    ...
    return mysql.connect(
        host=DB_HOST, port=DB_PORT, user=DB_USER, passwd=DB_PASSWORD, database=DB_NAME
    )
...
def get_user_id(username):
    ...
    cur.execute("select user_id from user where username = '{}'".format(username))
    rv = cur.fetchone()
    print(rv)
    ...
    return render_template(
        ...
        where message.flagged = 0 and message.author_id = user.user_id and (
            ...
            user.user_id in (select whom_id from follower
                                    where who_id = {}))
        order by message.pub_date desc limit {}""".format(
...
def user_timeline(username):
    ...
    if g.user:
        followed = (
            ...
            follower.who_id = {} and follower.whom_id = {}""".format(
...
def follow_user(username):
    ...
    cur = g.db.cursor(dictionary=True)
    cur.execute(
        "insert into follower (who_id, whom_id) values ({}, {})".format(
...
def unfollow_user(username):
    ...
    cur = g.db.cursor(dictionary=True)
    cur.execute(
        "delete from follower where who_id={} and whom_id={}".format(
            session["user_id"], whom_id
        )
    )
    g.db.commit()
